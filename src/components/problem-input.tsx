...
export function ProblemInput({ value, onChange, onSubmit, isLoading, error, image, onImageChange }: ProblemInputProps) {
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl border border-[var(--ion-border)] bg-white shadow-[var(--ion-shadow)] dark:bg-stone-900">
      ...
      <div className="space-y-3 p-5">
        ...
        {image ? (
          <div className="relative inline-block">
            <img src={image} alt="Pasted" className="max-h-32 rounded-lg border border-stone-200 dark:border-stone-700" />
            <button
              type="button"
              onClick={() => onImageChange(null)}
              className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full border border-stone-300 bg-white text-xs text-stone-500 hover:bg-stone-100 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-400"
            >&times;</button>
          </div>
        ) : (
      ...
      <div className="flex items-center justify-between border-t border-[var(--ion-border-light)] px-5 py-3 dark:border-[var(--ion-border)]">
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-lg bg-stone-950 px-5 py-2 text-xs font-semibold text-white transition-all hover:bg-stone-800 disabled:opacity-40 dark:bg-stone-100 dark:text-stone-950 dark:hover:bg-stone-300"
        >
          {isLoading ? (
      ...
      {error && (
        <div className="border-t border-[var(--ion-border-light)] px-5 py-3 dark:border-[var(--ion-border)]">
